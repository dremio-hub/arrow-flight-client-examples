...
func main() {
	...
	// TestableFlightClient is simple wrapper for flight.Client to allow for testing
	testableClient := &implementations.TestableFlightClient{Client: client}
	if err := run(config, testableClient, interfaces.WrapRecordReader); err != nil {
		log.Fatal(err)
	}
}
